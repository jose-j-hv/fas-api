...
def crear_usuario(usuario,db:Session):
    ...
    try:
        password_hash = Hash.hash_password(usuario["password"] ) 
...
def obtener_usuario(user_id, db:Session):
    ...
    if not usuario : 
        raise HTTPException(
            ...
            detail=f"No exsiste el usuario con el id: {user_id}"
        )
    return usuario
...
def eliminar_usuario(user_id,db:Session):
    ...
    if not usuario.first():
        raise HTTPException(
            status_code= status.HTTP_404_NOT_FOUND,
            detail=f"No exsiste el usuario con el id: {user_id} por lo tanto no se elimino"
        )
